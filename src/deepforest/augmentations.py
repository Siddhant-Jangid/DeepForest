...
validation that can be specified through configuration files or direct"""Augmentation module for DeepForest using kornia.
...
class PadTo(GeometricAugmentationBase2D):
    r"""Pad an image to a specified size without cropping if image is larger.

    Unlike kornia's PadTo which crops images larger than the target size,
    this implementation only pads images that are smaller than the target size.
    Images larger than the target size are left unchanged.

    Args:
        size: Target size as (height, width) tuple in pixels.
        pad_mode: Padding mode (constant, reflect, replicate, circular).
        pad_value: Fill value for constant padding mode.
        p: Probability of applying the transform.
        same_on_batch: Apply same transformation to all batch elements.
    ...
    def apply_transform(
        ...
        # Only pad if image is smaller than target size
...
_SUPPORTED_TRANSFORMS = {
    ...
    "PadTo": (PadTo, {"size": (800, 800), "pad_mode": "constant", "pad_value": 0}),
